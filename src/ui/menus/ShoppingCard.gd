...
signal purchase_requested(ids: Array)
...
func setup_offer(event: Dictionary, offer: Dictionary) -> void:
	_choose = int(offer["choose"])
	_items.assign(offer["items"])
	_result_text = str(event.get("result_after", ""))
	_selected.clear()
	_submitted = false
	if _phone == null: _build_shell()
	_build_content()
...
func _on_confirm() -> void:
	...
	var ids: Array = []
	for item in _items:
		if _selected.get(item["id"], false): ids.append(item["id"])
	if ids.size() != _choose:
		return
	_submitted = true
	_confirm.disabled = true
	purchase_requested.emit(ids)
func _layout() -> void:
	if _phone == null:
		return
	...
	if _phone.get_child_count() > 0:
		var artist := _phone.get_child(0) as Control
		artist.size = _phone.size
		artist.queue_redraw()
