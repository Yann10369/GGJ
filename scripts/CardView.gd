...
func apply_stack(pos: int, deck_size: Vector2) -> void:
	...
	var ch := maxf(minf(deck_size.y - 14.0, cw * CARD_RATIO), 120.0)
	size = Vector2(cw, ch)
...
func _build_content(number: int, total: int) -> void:
	...
	if _detailed:
		...
		Fonts.apply(desc, 13, Color("#6b6f7a"))
		vbox.add_child(desc)
...
func _mark(text: String, color: Color, rot: float) -> PanelContainer:
	...
	var label := Label.new()
	label.text = text
	Fonts.apply(label, 22, color, "serif-bold")
	panel.add_child(label)
	...
	if event is InputEventMouseButton:
		...
		if event.pressed:
			if _back_tween != null and _back_tween.is_valid():
				_back_tween.kill()
			_dragging = true
			_start = get_global_transform() * event.position
			_offset = Vector2.ZERO
			get_viewport().set_input_as_handled()
		elif _dragging:
			_dragging = false
			get_viewport().set_input_as_handled()
			released.emit(_offset.x)
	elif event is InputEventMouseMotion and _dragging:
		_offset = get_global_transform() * event.position - _start
		_apply_drag(_offset)
		get_viewport().set_input_as_handled()
